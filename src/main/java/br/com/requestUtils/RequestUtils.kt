...
object RequestUtils {
    ...
    var gson = GsonBuilder()
            ...
            .create()
    init {
        val teste = Prefs.getString("access_token", "")
        if(teste != "") {
            addHeader("Authorization",teste)
        }
    }
    ...
    inline fun <reified T> _get(url: String,liveData: MutableLiveData<T>,failed: MutableLiveData<Boolean>?=null){
        Log.i("Url","Esssa url " +url );
        baseRequest.url(url).get()
        startRequest(liveData,failed)
    }

    fun <T:Any> get(clazz:KClass<T>, url: String, liveData: MutableLiveData<T>, request:Request.Builder, failed: MutableLiveData<Boolean>?=null){
        request.url(url).get()
        startRequest(clazz,liveData,request,failed)
    }
    ...
    fun <T:Any> startRequest(clazz:KClass<T>, liveData: MutableLiveData<T>, request:Request.Builder, failed:MutableLiveData<Boolean>? = null){
        Log.i("passed","chegou aqui tbm")
        client.newCall(request.build()).enqueue(object : okhttp3.Callback {
            override fun onResponse(call: okhttp3.Call, response: okhttp3.Response) {
                ...
                Log.i("teste",content)
                if(response!!.code() < 300 && content != "") {
                    liveData.postValue(convert(content!!,clazz))
                }
                else if (failed != null){
                    failed?.postValue(true)
                }
...
fun <T:Any> convert(json: String, classe: KClass<T>):T{
    return GsonBuilder()
            .setLenient()
            .create().fromJson(json,classe.java)
}


inline fun <reified T> Gson.fromJson1(json: String) = this.fromJson<T>(json,object: TypeToken<T>(){}.type)
